...
final class DevelopersRoutes(connectionsService: ConnectionsService)(implicit conc: Concurrent[IO], async: Async[IO])
    extends Http4sDsl[IO] {

  val routes: HttpRoutes[IO] = Http4sServerInterpreter[IO].toRoutes(DevelopersRoutes.connectedRoute) {
    connectedInputDto =>
...
object DevelopersRoutes {
  ...
  val exampleErrors = ErrorsOutputDto(
    List(
  ...
  val connectedRoute: Endpoint[ConnectedInputDto, ErrorsOutputDto, ConnectedOutputDto, Any] = endpoint.get
    ...
    .out(
      jsonBody[ConnectedOutputDto]
        .description("JSON result")
        .example(exampleConnectedTrueOutputDto)
        .example(exampleConnectedFalseOutputDto)
    )
    .description(
